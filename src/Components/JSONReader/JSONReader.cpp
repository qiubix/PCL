...
void JSONReader::read() {
	...
	BOOST_FOREACH(boost::property_tree::ptree::value_type &v0, ptree_file_rgb){//clouds
		...
		BOOST_FOREACH(boost::property_tree::ptree::value_type &v, v0.second)//points
			{
				...
				try{
					...
					point_xyzrgbsift.x = boost::lexical_cast<float>(v.second.get_child("x").data());
					point_xyzrgbsift.y = boost::lexical_cast<float>(v.second.get_child("y").data());
					point_xyzrgbsift.z = boost::lexical_cast<float>(v.second.get_child("z").data());
